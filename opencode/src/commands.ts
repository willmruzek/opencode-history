...
export async function showMessageDiff(messageId?: string) {
  if (!messageId) {
    messageId = await opencode.window.showInputBox({
  ...
  if (!diff) {
    opencode.window.showInformationMessage('No file changes found for this message');
    return;
  }

  // Create a new document with the diff
  const doc = await opencode.workspace.openTextDocument({
    content: diff,
    language: 'diff'
  });
  await opencode.window.showTextDocument(doc);
}

export async function showSessionChanges(sessionId?: string) {
  if (!sessionId) {
    sessionId = await opencode.window.showInputBox({
  ...
  if (!changes || changes.length === 0) {
    opencode.window.showInformationMessage('No file changes found for this session');
    return;
  }
...
export async function showFileHistory(filePath?: string) {
  if (!filePath) {
    const editor = opencode.window.activeTextEditor;
    if (editor) {
      filePath = opencode.workspace.asRelativePath(editor.document.uri);
    } else {
      filePath = await opencode.window.showInputBox({
  ...
  if (!history || history.length === 0) {
    opencode.window.showInformationMessage(`No history found for: ${filePath}`);
    return;
  }
